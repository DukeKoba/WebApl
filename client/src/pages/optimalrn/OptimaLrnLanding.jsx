...
// ── App card icon (shield style matching app icons) ───────────
...
export default function OptimaLrnLanding() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      {/* ── Navbar ── */}
      <nav className="fixed top-0 inset-x-0 z-50 bg-black/95 backdrop-blur border-b border-white/10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm text-gray-400">
            <a href="#apps" className="hover:text-white transition-colors">アプリ</a>
        ...
        {menuOpen && (
          <div className="md:hidden bg-black border-t border-white/10 px-4 py-4 space-y-3 text-sm">
            {[['アプリ', '#apps'], ['特徴', '#features'], ['FAQ', '#faq']].map(([label, href]) => (
              <a key={href} href={href} className="block text-gray-300 hover:text-white py-1" onClick={() => setMenuOpen(false)}>
                {label}
              </a>
            ))}
      ...
      {/* ── Hero ── */}
      <section className="pt-24 pb-24 px-4 sm:px-6 bg-black text-white relative overflow-hidden">
        {/* Background glow */}
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <span className="inline-flex items-center gap-1.5 bg-orange-500/15 text-orange-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-6 border border-orange-500/20">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a
              href="#apps"
              className="inline-flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-400 text-white px-8 py-4 rounded-xl font-bold text-base transition-colors shadow-xl shadow-orange-900/30"
            >
          ...
          <p className="text-xs text-gray-600 mt-5">努力を科学する</p>
        </div>
      </section>
      ...
      {/* ── Features ── */}
      <section id="features" className="py-20 px-4 sm:px-6 bg-black text-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl sm:text-4xl font-extrabold mb-3">なぜ OptimaLrn なのか</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {features.map((f, i) => {
              const Icon = f.icon;
              return (
                <div key={i} className="flex gap-4 p-6 bg-white/5 rounded-2xl border border-white/10 hover:border-orange-500/40 transition-colors">
          ...
          <div className="grid sm:grid-cols-3 gap-8 relative">
            ...
            {[
              ...
              <div key={i} className="relative z-10">
                <div className="w-16 h-16 rounded-full bg-black text-orange-500 text-xl font-extrabold flex items-center justify-center mx-auto mb-4 shadow-lg">
                  {s.num}
                </div>
      ...
      <section className="py-20 px-4 sm:px-6 bg-black text-white text-center">
        <div className="max-w-2xl mx-auto">
          ...
          />
          <h2 className="text-3xl sm:text-4xl font-extrabold mb-4">努力を、科学しよう。</h2>
          <p className="text-gray-400 mb-8 text-base">目標の試験アプリを選んでダウンロード。</p>
          <a
            href="#apps"
            className="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-400 text-white px-10 py-4 rounded-xl font-bold text-base transition-colors shadow-lg shadow-orange-900/30"
          >
      ...
      {/* ── Footer ── */}
      <footer className="py-8 px-4 sm:px-6 bg-black border-t border-white/5 text-center">
        <img
          src="/optimalrn-logo.png"
          alt="OptimaLrn"
          ...
          style={{ filter: 'brightness(0) invert(1)' }}
        />
        <p className="text-xs text-gray-700">© 2026 OptimaLrn. Powered by Claude AI.</p>
        <button
          onClick={() => navigate('/')}
          className="mt-3 text-xs text-gray-700 hover:text-gray-400 underline transition-colors"
        >
          ← アプリ一覧に戻る
        </button>
      </footer>
