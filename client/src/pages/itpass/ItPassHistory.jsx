...
import { ArrowLeft, ShieldCheck, Twitter, Clock, CheckCircle, XCircle, FileText, Trash2 } from 'lucide-react';
...
const CONTENT_TYPE_LABELS = {
  ...
  database: 'データベース',
  tech_basics: '基礎理論',
  tech_computer: 'コンピュータ構成',
  strategy: 'ストラテジ系',
  management: 'マネジメント系',
  news_law: 'ニュース・法務',
};

export default function ItPassHistory() {
  const [posts, setPosts] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    authFetch('/itpass/posts')
      .then(r => r.json())
  ...
  const handleDelete = async (id) => {
    ...
    await authFetch(`/itpass/posts/${id}`, { method: 'DELETE' });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-4 py-4">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <Link to="/itpass" className="text-gray-400 hover:text-gray-600">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-bold text-lg text-gray-900">ITパスポート 投稿履歴</h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4 lg:p-6">
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <Link to="/itpass" className="text-cyan-600 text-sm hover:underline mt-2 inline-block">
              最初の投稿を生成する →
            </Link>
          </div>
        ) : (
