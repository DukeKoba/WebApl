...
export function generateSubsidyDraft(data) {
  const challenges = [...data.currentChallenges, ...(data.customChallenge ? [data.customChallenge] : [])];
  const tools = [...data.desiredTools, ...(data.customTool ? [data.customTool] : [])];
  ...
  const currentIssues = challenges.map((c, i) => `（${i + 1}）${c}`).join("\n") +
  ...
  return {
    ...
    expectedEffects,
    budgetEstimate,
    subsidyCategory: isHighTier ? "複数プロセス（4プロセス以上）" : "通常枠（1〜3プロセス）",
